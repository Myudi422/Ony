...
export default function ClaimPage({
  ...
  const [claimSuccess, setClaimSuccess] = useState<{ cardName: string; email: string } | null>(null)
  ...
  useEffect(() => {
    if (authStatus === 'authenticated' && session?.user && autoClaimParam && !isUnpaid && !autoClaimTriggered) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#fafafa] lg:bg-slate-100/60 flex flex-col justify-between text-slate-900 selection:bg-ony-blue selection:text-white antialiased">
      ...
      {/* ===== AUTO-CLAIM OVERLAY (after Google Login redirect) ===== */}
      {autoClaimParam && authStatus !== 'unauthenticated' && !claimSuccess && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white/95 backdrop-blur-sm animate-in fade-in duration-300">
          {claimingOwner || authStatus === 'loading' ? (
            // Loading state
            <div className="flex flex-col items-center gap-4 text-center px-6">
              <div className="w-16 h-16 rounded-full bg-blue-50 border-2 border-blue-100 flex items-center justify-center">
                <Loader2 size={28} className="animate-spin text-ony-blue" />
              </div>
              <div>
                <p className="font-bold text-slate-900 text-base font-display">
                  {authStatus === 'loading' ? 'Memverifikasi akun Google...' : 'Menghubungkan kartu ke akun Anda...'}
                </p>
                <p className="text-slate-500 text-xs mt-1">Harap tunggu sebentar.</p>
              </div>
            </div>
          ) : claimOwnerError ? (
            // Error state (auto-claim failed)
            <div className="flex flex-col items-center gap-4 text-center px-6 max-w-sm">
              ...
              <div>
                <p className="font-bold text-slate-900 text-base font-display mb-1">Gagal Mengklaim Kartu</p>
                <p className="text-slate-600 text-xs leading-relaxed">{claimOwnerError}</p>
              </div>
              <button
                type="button"
                onClick={() => {
                ...
                className="px-5 py-2.5 rounded-xl bg-slate-900 text-white text-xs font-bold font-display cursor-pointer"
              >
                Coba Lagi
              </button>
            </div>
                ...
                <div className="space-y-5 text-left">
                  ...
                  {isUnpaid ? (
                    ...
                    <div className="space-y-3">
                      {claimSuccess ? (
                        // === SUCCESS STATE ===
                        <div className="p-5 rounded-2xl bg-emerald-50 border border-emerald-200 text-center space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                          <div className="w-14 h-14 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center mx-auto">
                            <CheckCircle2 size={30} />
                          </div>
                          <div>
                            <h4 className="font-bold text-slate-900 text-base font-display mb-1">Kartu Berhasil Diklaim! 🎉</h4>
                            <p className="text-xs text-slate-600 leading-relaxed">
                              <span className="font-semibold text-slate-800">{claimSuccess.cardName}</span> sudah terdaftar ke akun{' '}
                              <span className="font-semibold text-ony-blue">{claimSuccess.email}</span>.
                            </p>
                          </div>
                          <div className="pt-1 space-y-2">
                            <Link
                              href={`/dashboard?claimed=${code}`}
                              className="w-full py-3 px-6 rounded-xl bg-ony-blue hover:bg-blue-700 text-white font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2 font-display"
                            >
                              <ShieldCheck size={16} />
                              <span>Buka Dashboard Saya</span>
                            </Link>
                            <p className="text-[11px] text-slate-400">
                              Kelola kartu, ubah link, dan pantau statistik tap di sini.
                            </p>
                          </div>
                        </div>
                      ) : (
